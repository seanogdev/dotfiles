...
`model: "haiku"`. Never use `fork` for this — a fork inherits your full
context, which defeats the point of keeping the file content out of it.

**Bulk read prompt.** Name the exact files or glob, and ask for a short,
structured answer, not the file contents:

> Read `src/api/*.ts`. Report, in bullet points: which files export a
> `retry` helper, and what its signature is. Do not quote file contents.

**Boilerplate write prompt.** Point at the pattern file and the new file's
purpose, and ask the subagent to write the file directly:

> Read `src/components/UserCard.tsx` as the pattern. Write
> `src/components/TeamCard.tsx` following the same structure, for a team
> with `name`, `members`, and `avatarUrl`. Report back only the file path
> and a one-line summary of what it does.

Take the subagent's summary at face value for routing decisions. Read the
file yourself only if you need to edit it, verify a claim, or the summary
