...
- `.Skillfile` excludes every skill without `github-repo` frontmatter. `content-writer` and `review-pr` have none, so they never appear in `.Skillfile` — they're private skills, not `gh skill`'s concern.
- The dotfiles repo is the exception to keeping project-scope and user-scope skills apart: its `.claude/skills/` holds the symlinks that stow projects into `$HOME`.
